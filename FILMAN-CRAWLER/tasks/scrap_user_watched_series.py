...
class Watched:
    def __init__(
        self, id_watched, id_filmweb, series_id, rate, comment, favorite, unix_timestamp
    ):
...
class Scraper:
    ...
    def scrap(self, task):
        ...
        user_already_watched = (
            f"{self.endpoint_url}/user/watched/series/all?id_filmweb={task.job}"
        )
        ...
        if user_already_watched_data is None:
            ...
            user_already_watched_data = list(
                map(lambda x: x["series_id"], user_already_watched_data)
            )
        ...
        for series_id in new_series_id:
            series_rate_data = self.fetch(
                f"https://www.filmweb.pl/api/v1/user/{task.job}/vote/serial/{series_id}"
            )
