...
# Only create task if celery is available
if celery_app is not None:
    ...
    # Mock task for when celery is not installed
    class MockTask:
        @staticmethod
        def delay(task_type: str, task_job: str):
            logging.warning(f"Celery not available. Would have queued: type={task_type}, job={task_job}")
