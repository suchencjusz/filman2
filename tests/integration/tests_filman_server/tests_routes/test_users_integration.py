...
def test_get_user(test_client):
    ...
    # get by filmweb_id
    for user_data in test_users_data:
        response = test_client.get(f"/users/get", params={"filmweb_id": user_data["filmweb_id"]})
        assert response.status_code == 200
    ...
    # get by discord_id
    for user_data in test_users_data:
        response = test_client.get(f"/users/get", params={"discord_id": user_data["discord_id"]})
        assert response.status_code == 200
    ...
    # get by discord_id and filmweb_id
    for user_data in test_users_data:
        response = test_client.get(
            f"/users/get",
            params={"discord_id": user_data["discord_id"], "filmweb_id": user_data["filmweb_id"]},
        )
        assert response.status_code == 200
    ...
    response = test_client.get(f"/users/get", params={"id": 10})
    assert response.status_code == 404

    response = test_client.get(f"/users/get", params={"filmweb_id": "non_existing"})
    assert response.status_code == 404

    response = test_client.get(f"/users/get", params={"discord_id": 123456789})
    assert response.status_code == 404

    response = test_client.get(f"/users/get", params={"discord_id": 123456789, "filmweb_id": "non_existing"})
    assert response.status_code == 404
...
def test_add_user_to_guild(test_client):
    ...
    # add user to guild that does not exist
    response = test_client.get(
        f"/users/add_to_guild",
        params={"discord_user_id": test_users_data[0]["discord_id"], "discord_guild_id": 2137},
    )
    assert response.status_code == 404

    # add user that does not exist to guild
    response = test_client.get(
        f"/users/add_to_guild",
        params={"discord_user_id": 2137, "discord_guild_id": guild_data["discord_guild_id"]},
    )
    assert response.status_code == 404

    # add non existing user to non existing guild
    response = test_client.get(
        f"/users/add_to_guild",
        params={"discord_user_id": 2137, "discord_guild_id": 2137},
    )
    assert response.status_code == 404
